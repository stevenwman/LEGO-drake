import pickle as pkl
import numpy as np
import matplotlib.pyplot as plt
from pathlib import Path
import tkinter as tk
from tkinter import filedialog
import matplotlib.colors as mcolors
import pandas as pd

def pick_folder():
    root = tk.Tk()
    root.withdraw()   # Hide the small tkinter window

    folder_path = filedialog.askdirectory(
        title="Select a folder",
        initialdir="/home/ckeefe/logs",
        mustexist=True
    )

    return folder_path





   
def plot_population_2d_from_hist(run_hist, iteration: int,
    save_path: str | Path | None = None,
):
    pop_hist = run_hist["pop_hist"]        # (iters, pop_size, num_dims)
    pop_rew_hist = run_hist["pop_rew_hist"]
    opt_params = run_hist["opt_params"]

    iters, pop_size, num_dims = pop_hist.shape
    if iteration < 0 or iteration >= iters:
        raise ValueError(f"iteration must be in [0, {iters-1}]")

    rewards = pop_rew_hist[iteration]

    # Debug: confirm values
    print("Reward range for this iteration:", np.min(rewards), np.max(rewards))

    # Global color scale across all iterations 
    vmin = float(np.min(pop_rew_hist))
    vmax = float(np.max(pop_rew_hist))

    norm = mcolors.Normalize(vmin=vmin, vmax=vmax)

    plt.figure()

    # MAIN SCATTER (this is the mappable for the colorbar)
    sc = plt.scatter(
        pop_hist[iteration, :, 0],
        pop_hist[iteration, :, 1],
        c=rewards,
        cmap="viridis",
        norm=norm,
    )

    # Mark the first individual (no norm, no c)
    plt.scatter(
        pop_hist[iteration, 0, 0],
        pop_hist[iteration, 0, 1],
        color="red",
        marker="X",
        s=60,
    )

    cbar = plt.colorbar(sc, label="Reward")
    # Optional: nicer formatting
    # from matplotlib.ticker import FormatStrFormatter
    # cbar.ax.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))

    plt.xlabel(opt_params.param_mask_keys[0])
    plt.ylabel(opt_params.param_mask_keys[1])
    plt.xlim(opt_params.param_min[0] * 0.9, opt_params.param_max[0] * 1.1)
    plt.ylim(opt_params.param_min[1] * 0.9, opt_params.param_max[1] * 1.1)
    plt.title(f"Population at Iteration {iteration}")
    plt.grid(True)

    if save_path is not None:
        plt.savefig(save_path, bbox_inches="tight")
    else:
        plt.show()

    plt.close()
    
def plot_full_population_2d(run_hist, save_path=None):
    import matplotlib.colors as mcolors
    import matplotlib.pyplot as plt
    import numpy as np
    from pathlib import Path

    pop_hist = run_hist["pop_hist"]        # (iters, pop_size, num_dims)
    pop_rew_hist = run_hist["pop_rew_hist"]
    opt_params = run_hist["opt_params"]

    iters, pop_size, num_dims = pop_hist.shape

    # Flatten population history
    all_x = pop_hist[:, :, 0].flatten()
    all_y = pop_hist[:, :, 1].flatten()
    all_rewards = pop_rew_hist.flatten()

    # Global color range
    vmin = float(np.min(all_rewards))
    vmax = float(np.max(all_rewards))
    norm = mcolors.Normalize(vmin=vmin, vmax=vmax)

    plt.figure(figsize=(8, 6))

    sc = plt.scatter(
        all_x,
        all_y,
        c=all_rewards,
        cmap="viridis",
        norm=norm,
        s=10,
        alpha=0.8
    )

    plt.colorbar(sc, label="Reward")
    plt.xlabel(opt_params.param_mask_keys[0])
    plt.ylabel(opt_params.param_mask_keys[1])
    plt.title("All NES Populations Across All Iterations")
    plt.grid(True)

    plt.xlim(opt_params.param_min[0] * 0.9, opt_params.param_max[0] * 1.1)
    plt.ylim(opt_params.param_min[1] * 0.9, opt_params.param_max[1] * 1.1)

    if save_path:
        plt.savefig(save_path, bbox_inches="tight")
    else:
        plt.show()

    plt.close()

"""
Load all values - 2D
Load best value at variable variable (if reward higher, replace) (also save other two variable)
Load all positions at starting amp freq

"""



def loadCSVFile(string_log_dir : Path):
    log_dir = Path(string_log_dir)
    index_path = log_dir / "index_with_metrics.csv"
    
    # Load CSV
    df = pd.read_csv(index_path)

        
    best_map = {}
    for _, row in df.iterrows():
        key = (row["X"], row["Y"])
    
        score = row["best_reward_final"]      # or reward, robustness, etc
        
        if key not in best_map:
            best_map[key] = row
        else:
            if score > best_map[key]["best_reward_final"]:
                best_map[key] = row
    #Could also use this instead, but much less clear df.loc[df.groupby(["X", "Y"])["score_stable"].idxmax()]  
    plot_XY_4D(best_map)
    plot_XY_heatmap(best_map)
            

def best_map_to_grid(best_map):
    import numpy as np

    X = np.array([k[0] for k in best_map.keys()])
    Y = np.array([k[1] for k in best_map.keys()])
    Z = np.array([row["best_reward_final"] for row in best_map.values()])

    # Unique sorted coordinates
    x_vals = np.unique(X)
    y_vals = np.unique(Y)

    nx = len(x_vals)
    ny = len(y_vals)

    # Create grid, fill with NaN initially
    Z_grid = np.full((ny, nx), np.nan, dtype=float)

    # Map from value -> index
    x_index = {x: i for i, x in enumerate(x_vals)}
    y_index = {y: j for j, y in enumerate(y_vals)}

    # Fill grid
    for (x, y), row in best_map.items():
        i = y_index[y]
        j = x_index[x]
        Z_grid[i, j] = row["best_reward_final"]

    return x_vals, y_vals, Z_grid

def plot_XY_heatmap(best_map, save_path=None):
    x_vals, y_vals, Z = best_map_to_grid(best_map)

    # For pcolormesh, we need bin edges, not centers
    # If grid is regular, midpoints between points work fine
    def make_edges(vals):
        vals = np.array(vals)
        d = np.diff(vals).mean()
        edges = np.concatenate([
            [vals[0] - d/2],
            vals[:-1] + d/2,
            [vals[-1] + d/2],
        ])
        return edges

    x_edges = make_edges(x_vals)
    y_edges = make_edges(y_vals)

    vmin = float(np.nanmin(Z))
    vmax = float(np.nanmax(Z))
    norm = mcolors.Normalize(vmin=vmin, vmax=vmax)

    plt.figure(figsize=(8, 6))

    # Note: pcolormesh expects (len(y_edges)-1, len(x_edges)-1) Z with y,x order
    mesh = plt.pcolormesh(
        x_edges,
        y_edges,
        Z,
        norm=norm,
        cmap="viridis",
        shading="auto",
    )

    cbar = plt.colorbar(mesh, label="Best Reward")  # or "Reward" if you prefer
    plt.xlabel("X Radius (m)")
    plt.ylabel("Y Radius (m)")
    plt.title("Best Reward across X–Y (max over boxX, boxY)")
    plt.grid(False)

    if save_path:
        plt.savefig(save_path, bbox_inches="tight")
    else:
        plt.show()

    plt.close()

def plot_XY_4D(best_map, save_path=None):
     #Now with values stored in the dictionary, convert back to arrays for plotting
    X_best = np.array([k[0] for k in best_map.keys()])
    Y_best = np.array([k[1] for k in best_map.keys()])
    #rewards = np.array([row["score_stable"] for row in best_map.values()])
    rewards = np.array([row["best_reward_final"] for row in best_map.values()])

    boxX_best = np.array([row["box_x"] for row in best_map.values()])
    boxY_best = np.array([row["box_y"] for row in best_map.values()])
    

    # Global color range
    vmin = float(np.min(rewards))
    vmax = float(np.max(rewards))
    norm = mcolors.Normalize(vmin=vmin, vmax=vmax)

    plt.figure(figsize=(8, 6))

    sc = plt.scatter(
        X_best,
        Y_best,
        c=rewards,
        cmap="viridis",
        norm=norm,
        s=10,
        alpha=0.8
    )

    plt.colorbar(sc, label="Reward")
    plt.xlabel(" X Radius (m)")
    plt.ylabel(" Y Radius (m)")
    plt.title("Optimized Reward at X and Y")
    plt.grid(True)

    plt.xlim(min(X_best), max(X_best))
    plt.ylim(min(Y_best),max(Y_best))

    if save_path:
        plt.savefig(save_path, bbox_inches="tight")
    else:
        plt.show()

    plt.close()
    
        
        



if __name__ == "__main__":
    optimizationFolder = pick_folder()
    loadCSVFile(optimizationFolder)
    #run_hist = load_run_hist(optimizationFolder)
    #plot_population_2d_from_hist(run_hist, 4)
    #test_version(run_hist, 4)
    #plot_full_population_2d(run_hist, optimizationFolder+ "/full_display")
    #run_NES_parallel(config)
