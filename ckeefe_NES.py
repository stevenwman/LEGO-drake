...
def run_NES_parallel(cfg: NES_Config):
    ...
    for iter in range(cfg.iterations):
        ...
        """
        for paramset in range(cfg.pop_size):
            for singleParam in range (num_dims):
                while( candidate_params[paramset,singleParam] > opt_params.param_max[singleParam] or candidate_params[paramset,singleParam] < opt_params.param_min[singleParam] ):
                    oldParam = candidate_params[paramset,singleParam]
                    newParam = curr_params[singleParam]*(1 + cfg.sigma*np.random.randn() ) 
                    print(f"{opt_params.param_min[singleParam]:.4f} < {oldParam:.4f} < {opt_params.param_max[singleParam]:.4f}  : NewVal = {newParam:.4f}")
                    candidate_params[paramset,singleParam] = newParam
        """
        ...
        match (NES_Config.greedFunction):
